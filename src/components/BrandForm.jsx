...
const BrandForm = ({ item, onSuccess, onCancel }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            let savedItem;
            if (item) {
                savedItem = await brandService.update(item.id, formData);
                if (!savedItem) {
                    savedItem = { ...item, ...formData };
                }
                onSuccess(savedItem, true);
            } else {
                savedItem = await brandService.create(formData);
                if (!savedItem) {
                    savedItem = { 
                        id: Date.now(),
                        ...formData 
            ...
            setError(error.message || 'Error al guardar la marca');
        } finally {
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="Ej: Yamaha, Suzuki, KTM"
                                maxLength="100"
                                autoFocus
                            />
                        </div>
